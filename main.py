...
- 提供 6 個主要功能選項：全面回測、回測交易、交易分析、自動化回測、前向分析(WFA)、可視化平台
- 協調各個模組的執行順序與數據流
...
- 選擇選項 5：前向分析（WFA，配置文件驅動，支援多配置批次執行）
...
def main():
    ...
    menu_items = [
        ...
        "",
        "[bold #dbac30]前向分析 (WFA) [/bold #dbac30]",
        "[bold white]5. 前向分析 Autorunner (配置文件驅動，支援多配置批次執行)[/bold white]",
        "",
